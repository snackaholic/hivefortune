...
form.addEventListener("submit", function (event) {
    ...
    if ((getUpvoters || getCommentators || getResteemer) && author != undefined && author != "") {
        ...
        if (getUpvoters) {
            ...
            hive.api.getActiveVotes(author, link, function (err, result) {
                if (result != undefined) {
        ...
        if (getCommentators) {
            expectedCallbackCounter++;
            hive.api.getContentReplies(author, link, function (err, result) {
                if (result != undefined) {
                    var length = result.length;
                    for (var i = 0; i < length; i++) {
        ...
        if (getResteemer) {
            expectedCallbackCounter++;
            hive.api.getRebloggedBy(author, link, function (err, result) {
                if (result != undefined) {
                    var length = result.length;
                    for (var i = 0; i < length; i++) {
